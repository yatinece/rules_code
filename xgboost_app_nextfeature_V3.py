...
class CatBoostRuleExtractor:
    def __init__(self, df, target_col='Class', feature_cols=None, 
                 categorical_features=None,
                 eval_metrics=['AUC', 'Accuracy', 'LogLoss'],
                 rule_direction='lt',
                 config=None):
        """
        df: DataFrame with data.
        ...
        for cat_col in self.categorical_features:
            if cat_col in self.X.columns:
                self.X[cat_col] = self.X[cat_col].astype(str)
    
    def _train_model(self, X, y, iterations, depth, eval_metric):
        """Helper method that trains a CatBoost model and returns the model."""
    ...
    def _apply_rule(self, X, rule_text):
        ...
        if feature in self.categorical_features:
            if operator == '==':
                return X[feature].astype(str) == threshold.strip("'\"")
            elif operator == '!=':
                return X[feature].astype(str) != threshold.strip("'\"")
            else:
                return pd.Series([False] * len(X), index=X.index)
    ...
    def _extract_rules_from_tree(self, model, tree_index, min_support):
        """Extract rules from a specific tree in the CatBoost model."""
        # Get the tree structure as JSON
        tree_structure = model.get_tree_json(tree_index)
        ...
        # Helper function to recursively extract rules from the tree
        def extract_from_node(node, path=None):
            if path is None:
                path = []
            
            # If this is a leaf node, return the path to it
            ...
            if feature_name in self.categorical_features:
                # CatBoost uses one-hot encoding internally
                if node['split_type'] == 'OneHotFeature':
                    # For categorical one-hot splits
                    value = node['split_one_hot_value']
                    
                    # Left path: feature == value
                    left_paths = extract_from_node(
                        node['left_subtree'], 
                        path + [(feature_name, '==', f"'{value}'")]
                    )
                    
                    # Right path: feature != value
                    right_paths = extract_from_node(
                        node['right_subtree'], 
                        path + [(feature_name, '!=', f"'{value}'")]
                    )
                    ...
                    # Handle other categorical split types if needed
                    return []
            else:
                # Numeric feature split
                split_boundary = node['split_border']
                
                # Left branch: feature < value
                left_paths = extract_from_node(
                    node['left_subtree'], 
                    path + [(feature_name, '<', split_boundary)]
                )
                
                # Right branch: feature >= value
                right_paths = extract_from_node(
                    node['right_subtree'], 
                    path + [(feature_name, '>=', split_boundary)]
                )
        ...
        all_paths = extract_from_node(tree_dict['tree_structure'])
        ...
        for path in all_paths:
            ...
            # Convert path to rule conditions
            conditions = []
            for feature, operator, value in path:
                conditions.append(f"{feature} {operator} {value}")
            
            # Create rule text
            rule_text = " AND ".join(conditions)
            
            # Create mask and get stats
            mask = self.apply_compound_rule(self.X, [f"{feature} {operator} {value}" for feature, operator, value in path])
            total = mask.sum()
            
            if total >= min_support:
                fraud_count = self.y[mask].sum()
                fraud_rate = fraud_count / total if total > 0 else 0
                
                rules.append({
                    'Tree': tree_index,
                    'Rule Text': rule_text,
                    'Suggested Rule': f"IF {rule_text} THEN FLAG",
                    'Total': total,
                    'Frauds': fraud_count,
                    'Fraud Rate': fraud_rate,
                    'Conditions': len(path),
                    'Path': path  # Store the path for later use
                })
        
        return rules
    
    def extract_rules_grid(self, iterations=100, depth=3, min_support=20):
        """
        Train CatBoost models with different parameters and extract rules.
        
        Args:
            iterations: Number of trees to build
            depth: Maximum depth of trees
            min_support: Minimum number of records to match a rule
            
        Returns:
            DataFrame of unique rules sorted by reward
        """
        all_rules = []
        progress_bar = st.progress(0)
        
        # Count total combinations for progress tracking
        total_combinations = len(self.eval_metrics)
        
        # Train models with different parameters
        for i, metric in enumerate(self.eval_metrics):
            st.write(f"Training CatBoost with eval_metric: {metric}")
            progress_bar.progress((i + 1) / total_combinations)
            
            # Train model
            model = self._train_model(self.X, self.y, iterations, depth, metric)
            
            # Extract rules from all trees
            for tree_idx in range(min(iterations, 20)):  # Limit to first 20 trees for efficiency
                tree_rules = self._extract_rules_from_tree(model, tree_idx, min_support)
        ...
        if all_rules:
            ...
            unique_rules = rules_df.drop_duplicates(subset=['Rule Text'])
            
            # Compute reward: sqrt(fraud_rate) * fraud_count
            def compute_reward(row):
                return np.sqrt(row['Fraud Rate']) * row['Frauds']
                
            unique_rules['Reward'] = unique_rules.apply(compute_reward, axis=1)
            unique_rules = unique_rules.sort_values(by=['Reward'], ascending=False).reset_index(drop=True)
            
            return unique_rules
        else:
            st.warning("No rules were extracted. Try different parameters.")
            return pd.DataFrame()

# Modify the original CompoundRuleBuilder to work with both extractors
...
# Modify the main function to include CatBoost support
def main():
    st.set_page_config(layout="wide", page_title="Tree Model Rule Extraction")
    st.title("Tree Model Rule Extraction Dashboard")
    st.write("""
    This dashboard allows you to extract rules from tree-based models and apply them iteratively to your data.
    Upload your data, configure parameters, extract rules, and apply them one by one to find patterns.
    You can now create compound rules with multiple conditions and use CatBoost for categorical features!
    """)
    def init_session_state():
        ...
        # Temporary dataframes for rule application
        if 'temp_matched_df' not in st.session_state:
            st.session_state['temp_matched_df'] = None
        if 'temp_remaining_df' not in st.session_state:
            st.session_state['temp_remaining_df'] = None
    ...
    if uploaded_file is not None:
        ...
        if target_col:
            ...
            # Add categorical feature selection if using CatBoost
            categorical_features = []
            if model_type == "CatBoost" and feature_cols:
                categorical_features = st.sidebar.multiselect(
                    "Categorical Features",
                    feature_cols,
                    key="cat_features_select"
                )
            ...
            if model_type == "XGBoost":
                ...
                eval_metrics = st.sidebar.multiselect(
                    "Evaluation Metrics",
                    ["AUC", "Accuracy", "LogLoss", "Precision", "Recall", "F1"],
                    default=["AUC"],
                    key="catboost_eval_metrics_select"
                )
                ...
                iterations = st.sidebar.slider("Number of Trees", 1, 200, 100, key="iterations_slider")
                max_depth = st.sidebar.slider("Maximum Tree Depth", 1, 10, 3, key="max_depth_slider")
            ...
            with main_area:
                ...
                if extract_rules_button:
                    ...
                    if df[target_col].nunique() > 2:
                        ...
                        with st.spinner(f"Extracting rules using {model_type}..."):
                            # Initialize appropriate extractor based on model type
                            if model_type == "XGBoost":
                                ...
                                extractor = CatBoostRuleExtractor(
                                    df,
                                    target_col=target_col,
                                    feature_cols=feature_cols,
                ...
                # Continue with the rest of the UI for rule display, filtering, application, etc.
                # (rest of the original code goes here...)
                
                # The rest of the function remains largely the same as the original,
                # continuing with rule display, compound rule building, and application
