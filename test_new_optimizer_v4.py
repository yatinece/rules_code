...
# Load dataset
base_dt = pd.read_csv(CONFIG['dataset_path'])
...
def discover_rules(max_rules=CONFIG['max_rules_per_run'], 
                  min_fraud_rate=CONFIG['min_fraud_rate'], 
    ...
    for iteration in range(max_iterations):
        ...
        if final_fraud_rate < min_fraud_rate * 0.8 or subset_size < CONFIG['min_subset_size_absolute']:
...
def multi_run_discovery(num_runs=CONFIG['num_runs']):
    all_rules = []
    all_ensembles = []
    
    for run in range(num_runs):
        print(f"\n\n=========== STARTING RUN {run+1} OF {num_runs} ===========")
        rules, ensembles = discover_rules(max_rules=CONFIG['max_rules_per_run'], 
                                        min_fraud_rate=CONFIG['min_fraud_rate'], 
                                        max_iterations=CONFIG['max_iterations'], 
...
try:
    all_discovered_rules, all_ensemble_rules = multi_run_discovery()
