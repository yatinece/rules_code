...
def discover_rules_with_config(config):
    ...
    Returns:
        tuple: (rules, fraud_detection_rate)
    """
...
def objective(trial):
    ...
    config = {
        ...
        # Ensemble Rule Parameters
        'ensemble_min_fraud_rate': BASE_CONFIG['ensemble_min_fraud_rate'], ##trial.suggest_float('ensemble_min_fraud_rate', 0.0005, 0.002)
        # Rule Discovery Parameters
        ...
        'num_runs': 2,  # Keep as 1 for optimization speed
...
def run_full_workflow(n_trials=30):
    ...
    best_rules, detection_rate = run_with_best_params(study, results_dir)
    
    print(f"\nFinal fraud detection rate: {detection_rate:.4%}")
    print(f"All results saved to {results_dir}")
    print("Optimization and evaluation complete!")
    
    return study, best_rules, results_dir
