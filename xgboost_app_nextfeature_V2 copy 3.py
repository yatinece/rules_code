...
# clusterting based on catagorical features
                    ...
                    if groups and len(groups) >= 5:
                        ...
                        if self.rule_direction == 'gt':
                            all_splits.append({
                                ...
                                'Gain': gain,
                                'Coverage': cover,
                                'Total': self.X[feature].gt(threshold).sum(),
                                'Frauds': self.y[self.X[feature].gt(threshold)].sum(),
                                'Fraud Rate': (self.y[self.X[feature].gt(threshold)].sum() / self.X[feature].gt(threshold).sum()) if self.X[feature].gt(threshold).sum() > 0 else 0
                            })
                            ...
                            all_splits.append({
                                ...
                                'Gain': gain,
                                'Coverage': cover,
                                'Total': self.X[feature].lt(threshold).sum(),
                                'Frauds': self.y[self.X[feature].lt(threshold)].sum(),
                                'Fraud Rate': (self.y[self.X[feature].lt(threshold)].sum() / self.X[feature].lt(threshold).sum()) if self.X[feature].lt(threshold).sum() > 0 else 0
                            })
                        else:  # both directions - add both unconditionally
                            all_splits.append({
                                'Tree Method': tm,
                                'Eval Metric': em,
                                'Tree Number': tree_idx,
...
# Main Streamlit app
def main():
    st.set_page_config(layout="wide", page_title="Rule Extraction")
    st.title("Rule Extraction Dashboard")
    st.write("""
    ...
    file_path = st.sidebar.text_input("Or enter path to CSV file:", value="")

    # Initialize df and a flag
    df = None
    path_used = False

    # Simulate upload if no file is uploaded but path is given
    if uploaded_file is None and file_path:
        if os.path.exists(file_path):
            ...
            st.sidebar.error("❌ File path doesn't exist. Please provide a valid path.")
            file_path = ""  # Reset path input in code logic (visual reset needs a workaround)
    ...
    if uploaded_file is not None:
        ...
        if 'target_col' not in st.session_state:
            st.session_state['target_col'] = df.columns[0]  # default value

        target_col = st.sidebar.selectbox(
            "Target Column", 
            df.columns.tolist(),
            index=df.columns.tolist().index(st.session_state['target_col']),
            key="target_col_select"
        )

        # Sync selectbox choice with session state
        st.session_state['target_col'] = target_col


        if 'categorical_transformation_done' not in st.session_state:
            st.session_state['categorical_transformation_done'] = False
        if st.session_state['target_col']:
            st.session_state['target_col'] = target_col
            
            # Feature columns selection (multi-select with all columns except target)

                # First: Categorical feature columns selection
            all_features = [col for col in df.columns if col != target_col]
            sorted_features = sorted(all_features, key=lambda x: not (x.endswith('_bin') or x.endswith('_cat')))
            no_cat_features = st.sidebar.checkbox("There are no categorical features in this dataset", value=False)
            categorical_feature_cols = []
            if not no_cat_features:
 # default value

 
                sorted_features = sorted(
                    [col for col in df.columns if col != target_col],
                    key=lambda x: not (x.endswith('_bin') or x.endswith('_cat'))
                )
                if 'categorical_feature_cols' not in st.session_state:
                    st.session_state['categorical_feature_cols'] = sorted_features[:min(len(sorted_features), 3)]
                categorical_feature_cols = st.sidebar.multiselect(
                    "Categorical Columns (used for encoding)",
                    sorted_features,
                    default=st.session_state['categorical_feature_cols'],
                    key="categorical_feature_cols_select"
                )

                category_extract_button = st.sidebar.button("categorical features selected", key="category_extract_button")
                st.session_state['categorical_feature_cols'] = categorical_feature_cols

                if category_extract_button:
                    progress_bar = st.progress(0, text="Starting transformation...")
                    progress_bar.progress(10, text="Initializing transformer...")
                    if len(categorical_feature_cols) > 0:
                        transformer = MultiFeatureCategoricalFraudTransformer(
                            category_cols=categorical_feature_cols,
                    ...
                    import time
                    time.sleep(5)                        
                    progress_bar.progress(70, text="Fitting transformer...")
                    import time
                    time.sleep(5)
                    transformed_df = transformer.fit_transform(df)
                    progress_bar.progress(80, text="Applying transformation...")
                    import time
                    time.sleep(5)
                    st.session_state['current_df'] = transformed_df
                    progress_bar.progress(100, text="Done!")
                    st.session_state['categorical_transformation_done'] = True
            else:
                st.sidebar.info("Skipping categorical transformation.")
            

            potential_features = [col for col in st.session_state['current_df'].columns if col != target_col and col not in st.session_state['categorical_feature_cols']]
                ...
                if extract_rules_button:
                    ...
                    if df[target_col].nunique() > 2:
                        ...
                        with st.spinner("Extracting rules..."):
                            ...
                            st.write(f"Feature under consideration: {extractor.feature_cols}")
                            # Extract rules
                        ...
                        if rule_builder.conditions:
                            ...
                            if len(filtered_df) >= min_support:
                                with st.spinner("Extracting new rules on filtered data..."):
                                    ...
                                    st.write(f"Feature under consideration: {new_extractor.feature_cols}")
                                    # Extract new rules
